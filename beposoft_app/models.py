...
def _qty_from_row(row):
    # Accepts 'quantity' primarily; tolerates common keys if client differs.
...
def _add_grv_racks_to_product(product, rows):
    ...
    - Creates missing slots with rack_stock initialized to 0.
    - Increments rack_stock by the given quantities.
    - Leaves rack_lock untouched.
    ...
    from beposoft_app.models import Products  # adjust import path if needed
    ...
    with transaction.atomic():
        ...
        # Create missing slots for additions
        ...
        for key, inc in adds.items():
            pr = index.get(key)
            before = _as_int(pr.get("rack_stock"))
            pr["rack_stock"] = before + inc
            changed = True

        if changed:
            p.rack_details = racks
            # This will also trigger your Products.save() recomputation of stock fields.
