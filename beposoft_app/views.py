...
class WarehouseSummaryView2(APIView):
    ...
    def get(self, request):
        try:
            ...
            def get_volume(obj):
                l = safe_decimal(obj.length)
                b = safe_decimal(obj.breadth)
                h = safe_decimal(obj.height)

                if l <= 0 or b <= 0 or h <= 0:
                    return Decimal("0")

                # raw volume = length × breadth × height
                # if your dimensions are in cm, this returns cm³
                return l * b * h

            def get_summary(qs):
                total_actual_weight_g = sum(
                    safe_decimal(v) for v in qs.values_list("actual_weight", flat=True)
                )
                total_parcel_amount = sum(
                    safe_decimal(v) for v in qs.values_list("parcel_amount", flat=True)
                )

                total_volume = sum(get_volume(w) for w in qs)
                total_weight_field = sum(
                    safe_decimal(w.weight) for w in qs
                )

                total_actual_weight_kg = (
                    total_actual_weight_g / Decimal("1000")
                    if total_actual_weight_g > 0 else Decimal("0")
                )

                average = (
                    total_parcel_amount / total_actual_weight_kg
                    if total_actual_weight_kg > 0 else Decimal("0")
                )

                return {
                    "total_boxes": qs.count(),
                    "total_actual_weight_g": float(total_actual_weight_g),
                    "total_actual_weight_kg": float(total_actual_weight_kg),
                    "total_parcel_amount": float(total_parcel_amount),
                    "average": float(average),
                    "total_volume": float(total_volume),
                    "total_weight_field": float(total_weight_field),
                }
            ...
            def get_service_summary(qs):
                ...
                for w in qs:
                    ...
                    if name not in result:
                        result[name] = {
                            "total_boxes": 0,
                            "total_actual_weight_g": Decimal("0"),
                            "total_parcel_amount": Decimal("0"),
                            "total_volume": Decimal("0"),
                            "total_weight_field": Decimal("0"),
                        }

                    result[name]["total_boxes"] += 1
                    result[name]["total_actual_weight_g"] += safe_decimal(w.actual_weight)
                    result[name]["total_parcel_amount"] += safe_decimal(w.parcel_amount)
                    result[name]["total_volume"] += get_volume(w)
                    result[name]["total_weight_field"] += safe_decimal(w.weight)

                for ps, data in result.items():
                    total_g = data["total_actual_weight_g"]
                    total_amt = data["total_parcel_amount"]

                    total_kg = total_g / Decimal("1000") if total_g > 0 else Decimal("0")
                    average = total_amt / total_kg if total_kg > 0 else Decimal("0")

                    data["total_actual_weight_g"] = float(data["total_actual_weight_g"])
                    data["total_parcel_amount"] = float(data["total_parcel_amount"])
                    data["total_volume"] = float(data["total_volume"])
                    data["total_weight_field"] = float(data["total_weight_field"])
                    data["total_actual_weight_kg"] = float(total_kg)
            ...
            for key in all_keys:
                all_services[key] = {
                    "today": today_services.get(key, default_summary.copy()),
                    "current_month": month_services.get(key, default_summary.copy()),
                    "last_30_days": days30_services.get(key, default_summary.copy())
                }
