...
class SalesAnalysisBySupervisorView(BaseTokenView):
    def get(self, request, supervisor_id, *args, **kwargs):
        try:
            search = request.GET.get("search", "").strip()
            call_status_filter = request.GET.get("call_status", "").strip()
            status_filter = request.GET.get("status", "").strip()
            ...
            sales_analysis = SalesAnalysis.objects.select_related(
                ...
                created_by__supervisor_id_id=supervisor_id
            ).order_by("-created_at")
            ...
            return paginator.get_paginated_response({
                "message": "Sales analysis fetched successfully by supervisor",
