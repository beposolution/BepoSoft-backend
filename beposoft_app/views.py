...
class OrderListWithoutPaginationView(BaseTokenView):
    ...
    def get(self, request):
        try:
            ...
            invoice_counts = orders.aggregate(
                invoice_created_count=Count(
                    "id",
                    filter=Q(status="Invoice Created")
                ),
                invoice_approved_count=Count(
                    "id",
                    filter=Q(status="Waiting For Confirmation")
                )
            )
            ...
            for idx, order in enumerate(orders):
                ...
                results[idx]["family_id"] = family.id if family else None
                results[idx]["family_name"] = family.name if family else None
                results[idx]["locked_by"] = (
                    order.locked_by.username if order.locked_by else None
                )
                results[idx]["locked_at"] = (
                    order.locked_at.isoformat() if order.locked_at else None
                )
...
class TodayOrderListView(BaseTokenView):
...
class FamilyOrderSummaryView(BaseTokenView):
    def get(self, request):
        try:
            ...
            def empty_payment_status_summary():
                return {
                    "today": {
            ...
            tq = (
                ...
                .annotate(
                    ...
                    today_total_amount=Coalesce(
                        Sum("total_amount", filter=Q(order_date=today_str)),
                        Value(0.0),
                    ...
                    t_paid_count=Count(
                        "id",
                        filter=Q(order_date=today_str, payment_status="paid")
                    ),
                    t_paid_total=Coalesce(
                        Sum("total_amount", filter=Q(order_date=today_str, payment_status="paid")),
                        Value(0.0),
                        output_field=FloatField()
                    ),
                    ...
                    t_cod_count=Count(
                        "id",
                        filter=Q(order_date=today_str, payment_status="COD")
                    ),
                    t_cod_total=Coalesce(
                        Sum("total_amount", filter=Q(order_date=today_str, payment_status="COD")),
                        Value(0.0),
                        output_field=FloatField()
                    ),

                    t_credit_count=Count(
                        "id",
                        filter=Q(order_date=today_str, payment_status="credit")
                    ),
                    t_credit_total=Coalesce(
                        Sum("total_amount", filter=Q(order_date=today_str, payment_status="credit")),
                        Value(0.0),
                        output_field=FloatField()
                    ),
                )
            ...
            mq = (
                ...
                .annotate(
                    ...
                    m_paid_count=Count(
                        "id",
                        filter=Q(
                            order_date__gte=month_start_str,
                            order_date__lte=today_str,
                            payment_status="paid"
                        )
                    ),
                    m_paid_total=Coalesce(
                        Sum(
                            "total_amount",
                            filter=Q(
                                order_date__gte=month_start_str,
            ...
            # -------- GRV return summary: TODAY + MONTH --------
            grv_q = (
                GRVModel.objects
                .exclude(order__status="Invoice Rejected")
                .values("order__family_id", "order__family__name")
                .annotate(
                    # TODAY COD RETURN
                    t_cod_return_invoice_count=Count(
                        "order_id",
                        filter=Q(date=today, remark="cod_return"),
                        distinct=True
                    ),
                    t_cod_return_total=Coalesce(
                        Sum("cod_amount", filter=Q(date=today, remark="cod_return")),
                        Value(Decimal("0.00")),
                        output_field=money_field
                    ),

                    # TODAY CASH / NORMAL RETURN
                    t_cash_return_invoice_count=Count(
                        "order_id",
                        filter=Q(date=today, remark="return"),
                        distinct=True
                    ),
                    t_cash_return_total=Coalesce(
                        Sum("price", filter=Q(date=today, remark="return")),
                        Value(Decimal("0.00")),
                        output_field=money_field
                    ),

                    # MONTH COD RETURN
                    m_cod_return_invoice_count=Count(
                        "order_id",
                        filter=Q(
                    ...
                    # MONTH CASH / NORMAL RETURN
                    m_cash_return_invoice_count=Count(
                        "order_id",
                        filter=Q(
                            date__gte=month_start,
                            date__lte=today,
            ...
            # -------- merge today + month + grv by family --------
            by_family = {}
            ...
            for r in tq:
                by_family[r["family_id"]] = {
                    ...
                    "payment_status_summary": {
                        "today": {
                            "paid": {
                                "count": r["t_paid_count"],
                                "total": float(r["t_paid_total"] or 0),
                            },
                            "COD": {
                                "count": r["t_cod_count"],
                                "total": float(r["t_cod_total"] or 0),
                            },
                            "credit": {
                                "count": r["t_credit_count"],
                                "total": float(r["t_credit_total"] or 0),
                            },
                        },
                        "month": {
                            "paid": {"count": 0, "total": 0.0},
                    ...
                    "grv_return_summary": empty_return_summary(),
                }
            ...
            for r in mq:
                row = by_family.setdefault(r["family_id"], {
                    ...
                    "month_count": 0,
                    "month_total_amount": 0.0,
                    "payment_status_summary": empty_payment_status_summary(),
                    "grv_return_summary": empty_return_summary(),
                })
                ...
                row["payment_status_summary"]["month"] = {
                    "paid": {
                        "count": r["m_paid_count"],
                        "total": float(r["m_paid_total"] or 0),
                    },
                    "COD": {
                        "count": r["m_cod_count"],
                        "total": float(r["m_cod_total"] or 0),
                    },
                    "credit": {
                        "count": r["m_credit_count"],
                        "total": float(r["m_credit_total"] or 0),
                    },
                }

            for r in grv_q:
                family_id = r["order__family_id"]

                row = by_family.setdefault(family_id, {
                    "family_id": family_id,
                    "family_name": r["order__family__name"],
                    "today_count": 0,
                    "today_total_amount": 0.0,
                    "month_count": 0,
                    "month_total_amount": 0.0,
                ...
                row["grv_return_summary"] = {
                    "today": {
                        "cod_return": {
                            "invoice_count": r["t_cod_return_invoice_count"] or 0,
                            "total": float(r["t_cod_return_total"] or 0),
                        },
                        "cash_return": {
                            "invoice_count": r["t_cash_return_invoice_count"] or 0,
                            "total": float(r["t_cash_return_total"] or 0),
                        },
                    },
                    "month": {
                        "cod_return": {
                            "invoice_count": r["m_cod_return_invoice_count"] or 0,
                            "total": float(r["m_cod_return_total"] or 0),
                        },
                        "cash_return": {
                            "invoice_count": r["m_cash_return_invoice_count"] or 0,
                            "total": float(r["m_cash_return_total"] or 0),
                        },
                    },
                }
            ...
            overall = {
                ...
                "payment_status_summary": {
                    "today": {
                        "paid": {
                            "count": sum(r["payment_status_summary"]["today"]["paid"]["count"] for r in results),
                            "total": float(sum(r["payment_status_summary"]["today"]["paid"]["total"] for r in results)),
                        },
                        "COD": {
                            "count": sum(r["payment_status_summary"]["today"]["COD"]["count"] for r in results),
                            "total": float(sum(r["payment_status_summary"]["today"]["COD"]["total"] for r in results)),
                        },
                        "credit": {
                            "count": sum(r["payment_status_summary"]["today"]["credit"]["count"] for r in results),
                            "total": float(sum(r["payment_status_summary"]["today"]["credit"]["total"] for r in results)),
                        },
                    },
                    "month": {
                        "paid": {
                            "count": sum(r["payment_status_summary"]["month"]["paid"]["count"] for r in results),
                            "total": float(sum(r["payment_status_summary"]["month"]["paid"]["total"] for r in results)),
                        },
                        "COD": {
                            "count": sum(r["payment_status_summary"]["month"]["COD"]["count"] for r in results),
                            "total": float(sum(r["payment_status_summary"]["month"]["COD"]["total"] for r in results)),
                        },
                        "credit": {
                            "count": sum(r["payment_status_summary"]["month"]["credit"]["count"] for r in results),
                ...
                "grv_return_summary": {
                    "today": {
                        "cod_return": {
                            "invoice_count": sum(
                                r["grv_return_summary"]["today"]["cod_return"]["invoice_count"]
                                for r in results
                            ),
                            "total": float(sum(
                                r["grv_return_summary"]["today"]["cod_return"]["total"]
                                for r in results
                            )),
                        },
                        "cash_return": {
                            "invoice_count": sum(
                                r["grv_return_summary"]["today"]["cash_return"]["invoice_count"]
                                for r in results
                            ),
                            "total": float(sum(
                                r["grv_return_summary"]["today"]["cash_return"]["total"]
                                for r in results
                            )),
                        },
                    },
                    "month": {
                        "cod_return": {
                            "invoice_count": sum(
                                r["grv_return_summary"]["month"]["cod_return"]["invoice_count"]
                                for r in results
                            ),
                            "total": float(sum(
                                r["grv_return_summary"]["month"]["cod_return"]["total"]
                                for r in results
                            )),
                        },
                        "cash_return": {
                            "invoice_count": sum(
                                r["grv_return_summary"]["month"]["cash_return"]["invoice_count"]
                                for r in results
                            ),
                            "total": float(sum(
                                r["grv_return_summary"]["month"]["cash_return"]["total"]
                                for r in results
                            )),
                        },
                    },
...
class DailyGoodsView(BaseTokenView):
    def get(self, request):
        try:
            ...
            for box_detail in warehouse:
                ...
                if shipped_date not in seen_dates:
                    ...
                    response_data.append({
                        ...
                        # Daily-wise top sold products retained here
            ...
            # ---------------------------------------------------------
            # FINAL RESPONSE
            # First item = filter summary and overall top products
            # Remaining items = existing daily-wise data with daily top products
            # ---------------------------------------------------------
            final_response = [
                {
                    "from_date": from_date,
            ...
            final_response.extend(response_data)
...
class PagenatedDailyGoodsView(BaseTokenView):
    def get(self, request):
        try:
            ...
            # ---------------------------------------------------------
            # BASE QUERYSET
            # Keep same base filter and same select_related structure.
            # ---------------------------------------------------------
            warehouse_base = Warehousedata.objects.select_related(
                "order",
                "order__billing_address",
                "order__family",
                "parcel_service",
            ...
            if from_date:
                ...
                warehouse_base = warehouse_base.filter(shipped_date__gte=parsed_from_date)
            ...
            # ---------------------------------------------------------
            filtered_unique_order_ids = warehouse_base.exclude(
            ...
            # ---------------------------------------------------------
            # REAL PAGINATION FIX
            # Paginate shipped dates first.
            # This avoids calculating all dates before pagination.
            # ---------------------------------------------------------
            date_queryset = warehouse_base.values(
                "shipped_date"
            ).distinct().order_by(
                "-shipped_date"
            )
            ...
            paginator = StandardPagination()
            paginated_dates = paginator.paginate_queryset(date_queryset, request)

            page_date_list = []

            for date_item in paginated_dates:
                page_date_list.append(date_item["shipped_date"])

            response_data = []

            if not page_date_list:
                return paginator.get_paginated_response({
                    "status": "success",
            ...
            # ---------------------------------------------------------
            # FETCH ONLY PAGINATED DATE BOXES
            # Same select_related fields.
            # ---------------------------------------------------------
            paginated_warehouse = warehouse_base.filter(
                shipped_date__in=page_date_list
            ).order_by(
                "-shipped_date",
                "-id"
            )

            boxes_by_date = defaultdict(list)
            order_ids_by_date = defaultdict(set)
            page_order_ids = set()

            for box in paginated_warehouse:
                shipped_date = box.shipped_date

                boxes_by_date[shipped_date].append(box)

                if box.order:
                    order_ids_by_date[shipped_date].add(box.order.id)
                    page_order_ids.add(box.order.id)

            # ---------------------------------------------------------
            # FETCH ORDER ITEMS ONLY ONCE FOR PAGINATED DATE ORDERS
            # Old code fetched OrderItem separately inside each date loop.
            # Calculation is same.
            # ---------------------------------------------------------
            order_items_by_order_id = defaultdict(list)

            if page_order_ids:
                page_order_items = OrderItem.objects.filter(
                ...
                for item in page_order_items:
                    order_items_by_order_id[item.order_id].append(item)

            # ---------------------------------------------------------
            # BUILD DATE-WISE RESPONSE
            # Same response keys and same calculations.
            # ---------------------------------------------------------
            for shipped_date in page_date_list:
                boxes_for_date = boxes_by_date.get(shipped_date, [])
                ...
                for order_id in unique_order_ids:
                    daily_order_items = order_items_by_order_id.get(order_id, [])
                ...
                total_boxes = len(boxes_for_date)
...
class StaffAttendanceView(BaseTokenView):
    ...
    def get(self, request):
        try:
            ...
            # Sales Department attendance from BDMOrderAnalysisStaff
            bdm_attendance_qs = BDMOrderAnalysisStaff.objects.select_related(
                "staff",
                "analysis"
            ).all()

            if start_date:
                bdm_attendance_qs = bdm_attendance_qs.filter(
                    analysis__attendance_date__gte=start_date
                )

            if end_date:
                bdm_attendance_qs = bdm_attendance_qs.filter(
                    analysis__attendance_date__lte=end_date
                )

            if member_id:
                bdm_attendance_qs = bdm_attendance_qs.filter(staff_id=member_id)

            bdm_attendance_qs = bdm_attendance_qs.order_by(
                "-analysis__attendance_date",
                "-id"
            )

            sales_date_wise_data = {}

            for attendance in bdm_attendance_qs:
                date_key = attendance.analysis.attendance_date.strftime("%Y-%m-%d")

                if date_key not in sales_date_wise_data:
                    sales_date_wise_data[date_key] = {
                        "attendance_date": date_key,
                        "present_count": 0,
                        "absent_count": 0,
                        "half_day_count": 0,
                ...
                if attendance.status == "present":
                    sales_date_wise_data[date_key]["present_count"] += 1
                elif attendance.status == "absent":
                    sales_date_wise_data[date_key]["absent_count"] += 1
                elif attendance.status == "half_day":
                    sales_date_wise_data[date_key]["half_day_count"] += 1

                sales_date_wise_data[date_key]["total_count"] += 1

                sales_date_wise_data[date_key]["attendance"].append({
                    "id": attendance.id,
                    "staff": attendance.staff.id,
                    "staff_name": attendance.staff.name if attendance.staff else None,
                    "attendance_date": date_key,
            ...
            sales_member_ids = list(
                bdm_attendance_qs.values_list("staff_id", flat=True).distinct()
            )
            ...
            data.append({
                ...
                "members_count": len(sales_member_ids),
                "date_wise_attendance": list(sales_date_wise_data.values())
            })
