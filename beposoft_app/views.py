...
# ---------------------------------------------
# Shopify Latest Orders API
# This API fetches the latest 20 orders from Shopify using GraphQL.
# It includes detailed information about each order, including line items and customer details.

from rest_framework.permissions import IsAuthenticated
class ShopifyLatestOrdersView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        shopify_url = (
            f"{settings.SHOPIFY_STORE_URL}"
            f"/admin/api/2025-01/graphql.json"
        )
