...
class LedgerSerializers(serializers.ModelSerializer):
    ...
    cod_return_amount = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    exchange_amount = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
...
class WarehouseDetailSerializer(serializers.ModelSerializer):
    country = serializers.CharField(source='country_code.country_code', read_only=True)
    class Meta:
        model = WareHouse
        ...
        cod_transfers = [
            {
                ...
                'amount': t['amount'],
                'expense_date': t['expense_date'],
                'purpose_of_payment': f"COD TRANSFER - {t['payment_receipt']}",
            }
...
class CODTransferViewSerializer(serializers.ModelSerializer):
    sender_bank_name = serializers.CharField(source='sender_bank.name', read_only=True)
    receiver_bank_name = serializers.CharField(source='receiver_bank.name', read_only=True)
    created_by_name = serializers.CharField(source='created_by.name', read_only=True)
...
class SalesTeamMemberDailyReportWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if self.instance:
            call_status = attrs.get('call_status', self.instance.call_status)
        ...
        if self.instance:
            if customer_name is None:
                customer_name = self.instance.customer_name
            if invoice is None:
        ...
        if call_status == 'active':
            if not customer_name or not str(customer_name).strip():
                raise serializers.ValidationError({
                    'customer_name': 'Customer name is required when call status is active.'
                })
            attrs['invoice'] = None

        elif call_status == 'productive':
            if not invoice:
                raise serializers.ValidationError({
                    'invoice': 'Invoice is required when call status is productive.'
                })
            attrs['customer_name'] = ""
    ...
    def create(self, validated_data):
        if validated_data.get('call_status') == 'productive':
            validated_data['customer_name'] = ""
        return super().create(validated_data)

    def update(self, instance, validated_data):
        if validated_data.get('call_status', instance.call_status) == 'productive':
            validated_data['customer_name'] = ""
        return super().update(instance, validated_data)
